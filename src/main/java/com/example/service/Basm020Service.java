...
public class Basm020Service {
	...
	public List<Basm020Class> search(String zip_code,String zip_area){
		String param = "";
		...
		Query q = em.createNativeQuery("select zip_code, mail_zipcode, zip_area "
	          	  +"from basm020 "
	          	  +"where 1 = 1"
	          	  +param,Basm020Class.class);
		if(!zip_code.isBlank()) {
		...
		List<Basm020Class> content = q.setMaxResults(300).getResultList();
		return content;
	}
}
